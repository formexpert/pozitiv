angular.module('df.common')
    ...
    .directive('autoComplete2', function ($timeout) {
        return function (scope, iElement, iAttrs) {

            if (typeof jQuery.ui == 'undefined') {
                alert('В directives.js -> directive(autoComplete) требуется Jquery.UI')
                return;
            }
        ...
        return {
            ...
            link: function ($scope, element, attrs) {
                ...
                if ($scope.autocomplete) {
                    ...
                    var source ;
                    if (typeof  $scope.autocomplete == 'object') {
                        source = function (request, response) {
                            var item;
                ...
                $scope.addTag = function () {
                    ...
                    var name = $scope.tagText;
                    if ($scope.tagText[$scope.tagText.length - 1] == ',') {
                        name = name.slice(0, -1);
                    }
                    name = name.trim();
                    if (name.length === 0) {
                    ...
                    var isset = false;
                    $.each($scope.inputTags, function (index, item) {
                        if (item.name.toLowerCase() == name.toLowerCase()) {
                            $scope.inputTags[index].delete = 0;
                            isset = true;
                        }
                    });
                    ...
                    $scope.inputTags.push(tag);
                    return $scope.tagText = "";
                };
                $scope.deleteTag = function (key) {
                    if ($scope.inputTags.length > 0 && $scope.tagText.length === 0 && key === undefined) {
                        if ($scope.inputTags[$scope.inputTags.length - 1].tag_id != 0) {
                            $scope.inputTags[$scope.inputTags.length - 1].delete = 1;
                        } else {
                            $scope.inputTags.pop();
                        }
                    } else {
                        if (key !== undefined) {
                            if ($scope.inputTags[key].tag_id != 0)
                                $scope.inputTags[key].delete = 1;
                            else {
                ...
                return element.bind("keyup", function (e) {
                    var key;
                    key = e.which;
                    if (key === 9 || key === 13 || key === 188 || key === 190) {
                        e.preventDefault();
                        return $scope.$apply('addTag()');
                    }
                });
            },
            template: "<div class='tag-input-ctn'><div ng-hide='tag.delete==1' class='input-tag' data-ng-repeat=\"tag in tagArray()\">{{tag.name}}<div class='delete-tag' data-ng-click='deleteTag($index)'>&times;</div></div><input type='text' data-ng-style='{width: inputWidth}' data-ng-model='tagText' placeholder='{{placeholder}}'/></div>"
        };
    })
